...
export function getTimeProgress(date: Date = new Date()): number {
  ...
  if (hour >= 6 && hour < 10) {
    // Morning: 6:00 - 10:00 (240 minutes)
    return (totalMinutes - 360) / 240
  }
  if (hour >= 10 && hour < 17) {
    // Day: 10:00 - 17:00 (420 minutes)
    return (totalMinutes - 600) / 420
  }
  if (hour >= 17 && hour < 20) {
    // Evening: 17:00 - 20:00 (180 minutes)
    return (totalMinutes - 1020) / 180
  }
  // Night: 20:00 - 6:00 (600 minutes)
  if (hour >= 20) {
    return (totalMinutes - 1200) / 600
  }
  return (totalMinutes + 240) / 600
}
