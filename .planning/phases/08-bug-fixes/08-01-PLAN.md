...
must_haves:
  ...
  artifacts:
    - path: "supabase/migrations/031_fix_quest_details.sql"
...
<task type="auto">
  ...
  <files>supabase/migrations/031_fix_quest_details.sql</files>
  <action>
Create a new migration file `supabase/migrations/031_fix_quest_details.sql` with:
...
5. Returns GuildQuestDetailed structure matching the TypeScript interface
  ...
  <verify>
1. File exists: `ls supabase/migrations/031_fix_quest_details.sql`
2. File contains the function: `grep -c "CREATE.*FUNCTION.*get_quest_details" supabase/migrations/031_fix_quest_details.sql` returns 1
3. File syntax valid (no obvious SQL errors)
  </verify>
  <done>Migration file 031_fix_quest_details.sql exists with get_quest_details function that returns quest with contributions array matching GuildQuestDetailed interface</done>
</task>
...
<success_criteria>
1. Migration file 031_fix_quest_details.sql exists with valid SQL
