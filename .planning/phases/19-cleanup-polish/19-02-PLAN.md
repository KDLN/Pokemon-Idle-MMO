...
must_haves:
  ...
  key_links:
    ...
    - from: "http://localhost:3000/storybook/"
      to: "public/storybook/index.html"
      via: "Next.js public folder serving"
---
...
# Requires clean lint state from Plan 01
...
After build, verify the route is accessible:
```bash
cd apps/web
npm run build
npm run start &
sleep 5
curl -s -o /dev/null -w "%{http_code}" http://localhost:3000/storybook/index.html
# Should return 200
...
2. `ls apps/web/public/storybook/index.html` exists after build
3. After `npm run build && npm start`, `curl -I http://localhost:3000/storybook/index.html` returns HTTP 200
4. Component stories render correctly with Modern theme (manual visual check)
  </verify>
  <done>public/storybook gitignored, /storybook route serves component documentation, HTTP 200 verified</done>
</task>
...
3. `npm run start` then curl `http://localhost:3000/storybook/index.html` returns HTTP 200
...
- /storybook route accessible in production (HTTP 200)
