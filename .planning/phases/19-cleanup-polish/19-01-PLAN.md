...
- `grep -r "theme-compare" apps/web/src/` returns no results (except possibly Next.js config if route was referenced)
  ...
  <done>MockGameScreen.tsx and theme-compare/page.tsx deleted with no orphaned references</done>
</task>

<task type="auto">
  <name>Task 2: Fix React purity errors (37 errors)</name>
  <files>
    - apps/web/src/components/game/EvolutionModal.tsx
    - apps/web/src/components/game/GymBattlePanel.tsx
    - apps/web/src/hooks/useBattleAnimation.ts
  ...
  <action>
Fix all `react-hooks/purity` and `react-hooks/set-state-in-effect` errors.

**EvolutionModal.tsx (Math.random in useMemo):**
Replace the impure sparklePositions useMemo with useState initialization:
```typescript
...
**GymBattlePanel.tsx (setState in useEffect):**
The issue is calling setBattlePhase/setBattleResult synchronously in useEffect. Refactor to use event-driven approach:
- Move initial state setting to a useLayoutEffect or initialize in useState
- For the `isGymOpen` effect, initialize state when gym opens using a ref to track if already initialized
- For battleResult effects, use the dependency array correctly to avoid synchronous cascades
...
**useBattleAnimation.ts (setState in useEffect):**
Similar pattern - the hook manages complex animation state. Solutions:
1. Use a ref to track pending state changes and batch them
2. Move synchronous state transitions to event handlers rather than effects
3. Use useReducer instead of multiple useState to batch state updates atomically

For the battle animation state machine, consider consolidating to useReducer:
```typescript
...
This consolidates all state changes into reducer, called from effects via dispatch().
  ...
  <verify>
Run `npm run lint -- --ignore-pattern "public/storybook/**" 2>&1 | grep -c "error"` returns 0 (or only warnings from deleted MockGameScreen if cache stale)
  </verify>
  <done>All 37 ESLint errors fixed - EvolutionModal uses seeded random, GymBattlePanel and useBattleAnimation use proper state management patterns</done>
</task>

<task type="auto">
  <name>Task 3: Fix unused variable warnings</name>
  <files>
    - apps/web/src/components/game/BoxPanel.tsx
    - apps/web/src/components/game/GameShell.tsx
    - apps/web/src/components/game/IVGradeBadge.tsx
  ...
  <action>
Fix unused variable warnings by either:
1. Removing the unused import/variable
2. Prefixing with underscore if intentionally unused (e.g., `_unused`)
3. Using the variable if it was meant to be used
...
**GymBattlePanel.tsx:**
Remove or use `playerHPPercent`, `gymHPPercent`
...
Remove unused RefObject import
  ...
  <verify>
Run `npm run lint -- --ignore-pattern "public/storybook/**" 2>&1 | grep "warning" | wc -l` - should be significantly reduced (only @next/next/no-img-element warnings should remain, which are acceptable for sprite images)
  </verify>
  <done>Unused variable warnings fixed - imports cleaned up, unused parameters prefixed with underscore</done>
</task>
