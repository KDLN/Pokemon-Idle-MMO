...
<task type="auto">
  ...
  <action>
Update TimeOfDayOverlay and timeOfDay.ts to implement 4-period sky system per CONTEXT.md:

1. Update timeOfDay.ts type names to match Mock terminology (dawn/day/dusk/night instead of morning/day/evening/night):
```typescript
...
```
Update TIME_COLORS keys accordingly (morning -> dawn, evening -> dusk).
Update getTimeProgress() period boundaries if needed.

2. Define sky gradients for each period in TimeOfDayOverlay:
...
- Drag a PokemonCard to reorder party
- Verify card picks up, drags smoothly, and drops in new position
- Verify party order persists after reorder

Towns show:
- Golden/amber ambient particles (different from route green particles)
  </verify>
  <done>PokemonCard has Mock styling with type colors, hover effects; towns have golden particles; drag-and-drop confirmed working</done>
</task>
...
- Drag-and-drop reordering works correctly after style changes
